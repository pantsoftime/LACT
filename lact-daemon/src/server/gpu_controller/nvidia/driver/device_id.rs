//! Resolve a GPU's RM device / subdevice instance from its PCI location.
//!
//! `/dev/nvidiaN` minors and RM device instances are not the same numbering:
//! on a multi-GPU host Panchovix found an RTX 5090 at minor 4 but RM instance
//! 5, and an Ada card the other way round, so allocating `NV01_DEVICE_0` with
//! the minor read the wrong card's private objects. The public root-client
//! queries (NVIDIA's `ctrl0000gpu.h`) map PCI identity to the instances that
//! `NV0080` / `NV2080` allocations expect. Ported from Panchovix/LACT commit
//! 738c149 (2026-09-14).
...
/// `(device_instance, subdevice_instance)` of the GPU at `pci`.
pub(super) fn resolve_gpu_instance(
    pci: &PciSlotInfo,
    mut query: impl FnMut(u32, &mut [u8]) -> anyhow::Result<()>,
) -> anyhow::Result<(u32, u32)> {
    // The RM query exposes domain / bus / slot but no PCI function; do not
    // match another function of a multifunction device by dropping it.
    ...
    for id in attached.as_chunks::<4>().0 {
        ...
        // NV0000_CTRL_GPU_GET_PCI_INFO_PARAMS: u32 gpuId, u32 domain, u16 bus, u16 slot.
...
mod tests {
    ...
    fn resolves_pci_identity_when_minor_and_rm_orders_differ() {
        // Ada at minor 5 / RM 4 and the 5090 at minor 4 / RM 5: enumeration
        // order must not select the device.
        let pci = PciSlotInfo {
            domain: 0,
            bus: 0x0d,
            dev: 0,
        ...
        let instances = resolve_gpu_instance(&pci, |cmd, data| {
            match cmd {
                ...
                GET_PCI_INFO => {
                    ...
                    data[8..10]
                        .copy_from_slice(&(if id == 0x2e00 { 0x2e_u16 } else { 0x0d }).to_ne_bytes());
                }
                GET_ID_INFO_V2 => {
