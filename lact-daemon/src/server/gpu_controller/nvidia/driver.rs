...
impl DriverHandle {
    pub fn open(minor_number: u32, pci_slot: &PciSlotInfo) -> anyhow::Result<Self> {
        ...
        let client_handle: NvHandle = unsafe {
            ...
            ensure!(client_request.status == 0, "Could not allocate Nvidia RM client");
            client_request.hObjectNew
        };

        // /dev/nvidiaN minors and RM device instances can be ordered
        // differently; resolve the RM object by PCI identity before opening
        // its device class (see device_id.rs).
    ...
    /// A control whose parameter block is a plain byte slice (private layouts
    /// with sizes that do not come from the bindings).
    pub(super) unsafe fn query_rm_control_bytes(&self, cmd: u32, params: &mut [u8]) -> anyhow::Result<()> {
        let mut request = NVOS54_PARAMETERS {
            ...
            flags: 0,
            params: params.as_mut_ptr().cast(),
            paramsSize: params.len().try_into().context("parameter block too large")?,
            status: 0,
        };
        unsafe {
