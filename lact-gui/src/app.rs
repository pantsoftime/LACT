...
impl AsyncComponent for AppModel {
    ...
    async fn init(
        ...
        // This fork's pages
        let adv_voltage_page = AdvVoltagePage::builder()
        ...
        let page_navigation = PageNavigation::detach(PageNavigationInit {
            pages: vec![
                ...
                (
                    PageId::AdvVoltage,
                    fl!(I18N, "adv-voltage-page"),
                    adv_voltage_page.widget().clone().upcast(),
                ),
                (
                    PageId::WireView,
                    fl!(I18N, "wireview-page"),
                    wireview_page.widget().clone().upcast(),
                ),
            ],
            parent: root.clone(),
...
impl AppModel {
    async fn handle_msg(
        ...
        match msg {
            ...
            AppMsg::SetPageVisible(page, visible) => {
                self.page_navigation
                    .emit(PageNavigationMsg::SetVisible(page, visible));
            }
