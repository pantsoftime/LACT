...
pub enum PageNavigationMsg {
    ...
    SyncSelection,
    CloseWindows,
    /// Show or hide a page's sidebar row and stack entry
    SetVisible(PageId, bool),
}
...
impl relm4::Component for PageNavigation {
    ...
    fn update_with_view(
        ...
        match msg {
            ...
            PageNavigationMsg::SetVisible(id, visible) => {
                if let Some(page) = self.pages.iter().find(|page| page.model().init.id == id) {
                    page.widgets().row.set_visible(visible);
                    widgets.stack.page(page.widget()).set_visible(visible);
                }
            }
        }
    }
