...
| Clock domain V/F curves | Read-only chart of the XBAR, SYS, video and PWRCLK curves (127 points each, on the MSVDD rail) with GPC switchable in, so the effect of the offsets above can be seen. | RM `CLK_VF_POINTS` info + status |
