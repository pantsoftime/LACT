...
| Power limit below the vBIOS minimum | The Power limit card's range starts at 30 W: under NVML's minimum the cap goes through the ordinary client of the RM power-policy group, which accepts and holds such requests (ported from Panchovix/LACT). A lower route only; the vBIOS maximum stands. | RM `0x2080a630/a632/e633`, verified against NVML before use |
| Clock domain V/F curves | Read-only chart of the XBAR, SYS, video and PWRCLK curves (127 points each, on the MSVDD rail) and GPC, all shown by default, so the effect of the offsets above can be seen. | RM `CLK_VF_POINTS` info + status |
| WireView II page | Thermal Grizzly WireView Pro II power meter on the 12V-2x6 cable: live power, current and per-pin bars scaled to its limits, faults, and every device setting (protection, fan, display, theme) with backup / read-back / flash-verify writes. Appears in the sidebar only when the device is plugged in. | USB CDC-ACM serial protocol in the daemon |
